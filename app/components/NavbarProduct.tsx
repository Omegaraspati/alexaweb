...
const NavbarProduct = () => {
  ...
  return (
    <nav
    className={`fixed top-0 w-full z-10 transition-colors duration-700  ${isScrolled ? " bg-blue-700 shadow-lg" : "bg-black"}`}>
    <div className='flex items-center'>
      <div className='logoalexa'>
        <Link href="/">
          <img src='/images/logo.png' alt='logo' className='pl-10 w-60' />
        </Link>
      </div>
      ...
      <div className="container max-md:hidden flex justify-end w-full items-center">
        <ul className="nav-links text-3xl text-white font-semibold uppercase flex  gap-8 ">
          ...
          <li className=' text-orange-700 underline text-3xl'><Link href={"/Products"}>Products</Link></li>
